...
function getCategories() {
    ...
    $query = "SELECT * from kategoritablo";
...
function getCoursesByFilters($categoryId, $keyword, $page) {
    ...
    $query = "";
    if(!empty($categoryId)) {
        $query = "from kurs_kategori kc INNER JOIN kurstablo k on kc.kurs_id=k.id WHERE kc.kategori_id=$categoryId and onay=1";
    } else {
        $query = "from kurstablo WHERE onay=1";
    }
...
function getCoursesByKeyword($q) {
    ...
    $query = "SELECT * from kurstablo WHERE baslik LIKE '%$q%' or altbaslik LIKE '%$q%'";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function getCategoriesByCourseId(int $courseId) {
    include "ayar.php";
    $query = "SELECT * FROM `kurs_kategori` kc inner join kategoritablo c on kc.kategori_id = c.id WHERE kc.kurs_id=$courseId";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function getCategoryById(int $id) {
    ...
    $query = "SELECT * from kategoritablo WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function getCourseById(int $id) {
    ...
    $query = "SELECT * from kurstablo WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function getCoursesByCategoryId(int $id) {
    ...
    $query = "SELECT * from kurs_kategori kc INNER JOIN kurstablo k on kc.kurs_id=k.id WHERE kc.kategori_id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function editCategory(int $id, string $category) {
    ...
    $query = "UPDATE kategoritablo SET kategori_adi='$category' WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function editCourse(int $id, string $baslik, string $altBaslik,string $aciklama, string $resim,int $onay,int $anasayfa) {
    ...
    $query = "UPDATE kurstablo SET baslik='$baslik', altbaslik='$altBaslik',aciklama='$aciklama',resim='$resim',onay=$onay,anasayfa=$anasayfa WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function deleteCategory(int $id) {
    ...
    $query = "DELETE FROM kategoritablo WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function deleteCourse(int $id) {
    ...
    $query = "DELETE FROM kurstablo WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
}
...
function createCategory(string $kategori) {
    ...
    $query = "INSERT INTO kategoritablo(kategori_adi) VALUES (?)";
...
function createCourse(string $baslik, string $altBaslik,string $aciklama, string $resim,int $yorumSayisi = 0, int $begeniSayisi=0,int $onay=0) {
    ...
    $query = "INSERT INTO kurstablo(baslik,altbaslik,aciklama,resim,yorumSayisi,begeniSayisi,onay) VALUES (?,?,?,?,?,?,?)";
    $stmt = mysqli_prepare($baglanti,$query);
